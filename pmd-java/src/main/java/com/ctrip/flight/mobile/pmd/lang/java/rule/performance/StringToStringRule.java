 ...
 */
@Deprecated
public class StringToStringRule extends FlightJavaRule {
        ...
        if (isStringVariableDeclarator(varId)) {
            for (NameOccurrence varUsage : varId.getUsages()) {
                ...
                if (isToStringOnStringCall(varId, qualifier)) {
                    addViolationWithPrecisePosition(data, varUsage.getLocation(), STRING_TO_STRING_VIOLATION_MSG);
                }
            }
        }
        return data;
            ...
            for (int callIndex = 2; callIndex < primaryExpr.getNumChildren(); callIndex++) {
                ...
                if (isToStringMethodCall(methodCall)) {
                    ...
                    if (calledMethodReturnsString(prevMethodCall, prevMethodCallArgs)) {
                        addViolationWithPrecisePosition(data, methodCall, STRING_TO_STRING_VIOLATION_MSG);
                    }
                }
            }
        }
    ...
    public Object visit(ASTPrimarySuffix node, Object data) {
        if (isToStringMethodCall(node) && node.getIndexInParent() > 0) {
            ...
            if (previousSibling instanceof ASTPrimaryPrefix && TypeTestUtil.isA(String.class,
                    (ASTPrimaryPrefix) previousSibling)) {
                addViolationWithPrecisePosition(data, node, STRING_TO_STRING_VIOLATION_MSG);
            }
        }
    ...
    private static final String STRING_TO_STRING_VIOLATION_MSG =
            "java.performance.StringToStringRule.violation.msg";
}
