 ...
 */
public class UnusedAssignmentRule extends FlightJavaRule {

    private static final String UN_USED_ASSIGNMENT_VIOLATION_MSG =
            "java.best.practices.UnusedAssignmentRule.violation.msg";
    ...
    private void reportFinished(GlobalAlgoState result, RuleContext ruleCtx) {
        if (result.usedAssignments.size() < result.allAssignments.size()) {
            ...
            for (AssignmentEntry entry : unused) {
                ...
                addViolationWithPrecisePosition(ruleCtx, entry.rhs, UN_USED_ASSIGNMENT_VIOLATION_MSG,
                        entry.var.getName());
            }
        }
    }
    ...
    private static class ReachingDefsVisitor extends JavaParserVisitorAdapter {
         ...
         *
         */
        private SpanInfo linkConditional(SpanInfo before, JavaNode condition, SpanInfo thenState, SpanInfo elseState,
                                         boolean isTopLevel) {
            if (condition == null) {
                return before;
            }
        ...
        private void maybeThrowUncheckedExceptions(ASTPrimaryExpression e, SpanInfo state) {
            ...
            for (JavaNode child : e.children()) {
                if (child instanceof ASTPrimarySuffix && ((ASTPrimarySuffix) child).isArguments()
                    ...
                    state.abruptCompletionByThrow(true); // this is a noop if we're outside a try block that has
                    // catch/finally
                }
            }
        }
        ...
        private ASTVariableDeclaratorId findVar(Scope scope, boolean isField, String name) {
            ...
            while (scope != null) {
                ...
                if (result != null) {
                    if (scope instanceof ClassScope && scope != enclosingClassScope) { // NOPMD
                        // CompareObjectsWithEqual this is what we want
                        // don't handle fields
                        return null;
                    }
                    return result;
