...
import com.ctrip.flight.mobile.pmd.lang.java.rule.FlightJUnitRule;
 ...
 */
public class JUnitTestsShouldIncludeAssertRule extends FlightJUnitRule {
        ...
        if (isJUnitMethod(method, data)) {
            if (!isExpectAnnotated(method.getParent())) {
                ...
                if (!containsExpectOrAssert(method.getBody(), expectables, variables)) {
                    addViolationWithPrecisePosition(data, method, JUNIT_TESTS_SHOULD_INCLUDE_ASSERT_VIOLATION_MSG);
                }
            }
        }
     ...
     *
     * @param classScope The class scope to search for
        ...
        if (expression != null) {
            ...
            if (pe != null) {
                ...
                if (name != null) {
                    ...
                    // 这段代码暂时注销，这里会强制判断必须是SoftAssertions，但是变量可以自己命名,所以variables.get(varName).getDeclaratorId()
                    // 拿到的是自己声明的变量名，TypeTestUtil.isA的比对不会通过
                    // && TypeTestUtil.isA("org.assertj.core.api.AbstractSoftAssertions",
                    // variables.get(varName).getDeclaratorId());
    ...
    private static final String JUNIT_TESTS_SHOULD_INCLUDE_ASSERT_VIOLATION_MSG =
            "java.best.practices.JUnitTestsShouldIncludeAssertRule.violation.msg";
}
