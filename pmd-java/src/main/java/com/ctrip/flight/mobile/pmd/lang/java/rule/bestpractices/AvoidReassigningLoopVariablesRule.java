 ...
 */
public class AvoidReassigningLoopVariablesRule extends FlightJavaRule {
            ...
            if (forReassign != ForReassignOption.ALLOW) {
                ...
                if (forReassign == ForReassignOption.SKIP) {
                    ...
                    checkIncrementAndDecrement(data, loopVariables, loopBody,
                            IgnoreFlags.IGNORE_CONDITIONAL);
                } else {
     ...
     */
    private void checkAssignExceptIncrement(Object data, Set<String> loopVariables, ASTStatement loopBody,
     ...
     * @param ignoreFlags which statements should be ignored
     */
    private void checkIncrementAndDecrement(Object data, Set<String> loopVariables, ASTStatement loopBody,
                                            IgnoreFlags... ignoreFlags) {
        ...
        for (ASTPostfixExpression expression : loopBody.findDescendantsOfType(ASTPostfixExpression.class)) {
            ...
            checkVariable(data, loopVariables,
                    singleVariableName(expression.getFirstDescendantOfType(ASTPrimaryExpression.class)));
        }
     ...
     * @param ignoreFlags    which statements should be ignored
     */
    private void checkAssignments(Object data, Set<String> loopVariables, ASTStatement loopBody,
                                  boolean checkIncrement, IgnoreFlags... ignoreFlags) {
        for (ASTAssignmentOperator operator : loopBody.findDescendantsOfType(ASTAssignmentOperator.class)) {
            ...
            final ASTPrimaryExpression primaryExpression =
                    operator.getParent().getFirstChildOfType(ASTPrimaryExpression.class);
    ...
    private boolean ignoreNode(Node node, ASTStatement loopBody, IgnoreFlags... ignoreFlags) {
        ...
        final boolean ignoredFirstStatement =
                ignoreFlagsList.contains(IgnoreFlags.IGNORE_FIRST) && isFirstStatementInBlock(node, loopBody);

        // ignore conditionally executed statement
        final boolean ignoredConditional =
                ignoreFlagsList.contains(IgnoreFlags.IGNORE_CONDITIONAL) && isConditionallyExecuted(node, loopBody);
    ...
    private boolean isConditionallyExecuted(Node node, ASTStatement loopBody) {
        ...
        while (checkNode.getParent() != null && !checkNode.getParent().equals(loopBody)) {
            ...
            if (parent instanceof ASTIfStatement || parent instanceof ASTSwitchStatement
                    || parent instanceof ASTWhileStatement || parent instanceof ASTDoStatement) {
            ...
            if (parent instanceof ASTForStatement) {
                return !(checkNode instanceof ASTForInit || checkNode instanceof ASTExpression
                        || checkNode instanceof ASTForUpdate);
            }
            checkNode = parent;
        }
    ...
    private void checkVariable(Object data, Set<String> loopVariables, AbstractNode node) {
        if (node != null && loopVariables.contains(node.getImage())) {
            addViolationWithPrecisePosition(data, node, AVOID_REASSIGNING_LOOP_VARIABLES_VIOLATION_MSG,
                    node.getImage());
        }
    }
    ...
    private static final String AVOID_REASSIGNING_LOOP_VARIABLES_VIOLATION_MSG =
            "java.best.practices.AvoidReassigningLoopVariablesRule.violation.msg";
}
