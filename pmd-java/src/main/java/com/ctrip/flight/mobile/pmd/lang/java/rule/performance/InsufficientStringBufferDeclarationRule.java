 ...
 */
public class InsufficientStringBufferDeclarationRule extends FlightJavaRule {
    ...
    public Object visit(ASTVariableDeclaratorId node, Object data) {
        if (node.getNameDeclaration() == null
                || !FlightRuleUtils.isStringBuilderOrBuffer(node)) {
            return data;
        }
        Node rootNode = node;
        ...
        for (NameOccurrence no : usage) {
            ...
            if (!FlightRuleUtils.isInStringBufferOperationChain(n, "append")) {
                ...
                if (!jno.isOnLeftHandSide()
                        && !(FlightRuleUtils.isInStringBufferOperationChain(n, "setLength")
                        || FlightRuleUtils.isInStringBufferOperationChain(n, "ensureCapacity"))) {
                    continue;
                }
    ...
    private void reportViolation(Object data, ASTVariableDeclaratorId instance, Node reportNode, int capacity,
        ...
        addViolationWithPrecisePosition(data, reportNode, INEFFICIENT_STRING_BUFFER_DECLARATION_VIOLATION_MSG, typeName,
                capacity, anticipatedLength);
    }
     ...
     *
     * @param node The node we're looking for a parent of
    ...
    private static final String INEFFICIENT_STRING_BUFFER_DECLARATION_VIOLATION_MSG =
            "java.performance.InsufficientStringBufferDeclarationRule.violation.msg";
}
