...
public final class PmdAnalysis implements AutoCloseable {
    ...
    void encourageToUseIncrementalAnalysis(final PMDConfiguration configuration) {
    ...
    Report performAnalysisImpl(List<DataSource> sortedFiles) {
        try (TimedOperation ignored = TimeTracker.startOperation(TimedOperationCategory.FILE_PROCESSING)) {
            ...
            newFileProcessor(configuration).processFiles(new net.sourceforge.pmd.RuleSets(convertPmdRuleSets(ruleSets)),
                    sortedFiles, ctx, renderers);
            configuration.getAnalysisCache().persist();
            return report;
        }
    }

    private List<net.sourceforge.pmd.RuleSet> convertPmdRuleSets(List<RuleSet> ruleSets) {
        if (!ruleSets.isEmpty()) {
            List<net.sourceforge.pmd.RuleSet> pmdRuleSetList = new ArrayList<>(ruleSets.size());
            for (RuleSet ruleSet : ruleSets) {
                net.sourceforge.pmd.RuleSet pmdRuleSet =
                        net.sourceforge.pmd.RuleSet.create(ruleSet.getName(),
                                ruleSet.getDescription(),
                                ruleSet.getFileName(),
                                ruleSet.getFileExclusions(),
                                ruleSet.getFileInclusions(),
                                ruleSet.getRules());
                pmdRuleSetList.add(pmdRuleSet);
            }
            return pmdRuleSetList;
        } else {
            ...
            for (final Rule rule : ruleSet.getRules()) {
                ...
                if (!languages.contains(ruleLanguage)) {
                    ...
                    if (RuleSet.applies(rule, version)) {
                        ...
                        reporter.trace("Using {0} version ''{1}''", version.getLanguage().getName(),
